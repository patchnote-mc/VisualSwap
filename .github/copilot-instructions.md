...
* `mc_decompiled/sources/26.2/common_src/` and
  ...
  26.2 source (Mojang mappings), pre-decompiled with CFR and committed
  alongside `common.jar` / `client.jar`.
  NOTE: `mc_decompiled/sources/26.2/` is now a **git submodule** (see
  `.gitmodules`, remote `patchnote-mc/mc_decompiled`) tracking branch `26.2`.
  Initialize/update it via `mc_decompiled/setup.sh` (or `git submodule update
  --init --recursive --depth 1`). They are NOT produced by `./gradlew genSources`.
