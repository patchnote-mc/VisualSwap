...
/// its widgets in {@code init()} and draws its panel chrome in {@link #renderPanel}.
public abstract class Modal extends Screen
{
    ...
    @Override
    public void render(@NonNull GuiGraphics g, int mouseX, int mouseY, float a)
    {
        ...
        renderPanel(g);
        g.nextStratum();
        super.render(g, mouseX, mouseY, a);   // this modal's own widgets, on top
    }

    /// Draw the modal's panel chrome (background + text) above the scrim and beneath the widgets.
    protected abstract void renderPanel(GuiGraphics g);
